...
describe("stars catalog", () => {
  it("gives every product a unique, stable identifier and a positive price", () => {
    const all = [...STAR_PRODUCTS, STAR_TEST_PRODUCT];
  ...
  it("keeps the 1-Star verification purchase available but out of the merchandised catalog", () => {
    ...
    expect(STAR_PRODUCTS.some((p) => p.id === STAR_TEST_PRODUCT.id)).toBe(false);
  ...
  it("scales credit drops so bigger packs are never worse value per Star", () => {
    const drops = STAR_PRODUCTS.filter((p) => p.section === "credits" && p.credits).sort(
      (a, b) => a.stars - b.stars,
    );
  ...
  it("marks Stars-exclusive items and exposes dual pricing where offered", () => {
    ...
    const exclusive = STAR_PRODUCTS.flatMap((p) => p.itemIds ?? []).filter((id) => isStarsOnlyItem(id));
    exclusive.forEach((id) => expect(isStarsOnlyItem(id)).toBe(true));
  });
});
...
describe("unbounded scaling", () => {
  it("formats huge hashrate and credit values without truncating to a cap", () => {
    expect(fmt(1_500)).toMatch(/1\.5K/);
    expect(fmt(2_500_000_000)).toMatch(/2\.5B/);
    expect(fmt(4.2e15)).toMatch(/4\.2P/);
